    ...
    <div class="page" id="page-branch">
      <div class="chart-grid-31">
        <div class="chart-card">
          ...
          <div id="chart-branch-dot" style="height:auto;min-height:460px"></div>
        </div>
        <div>
...
function navigate(el){{
  ...
  // Resize semua Plotly chart di halaman aktif agar tidak render ukuran 0
  setTimeout(()=>{{
    const activePage=document.getElementById('page-'+state.page);
    if(activePage) activePage.querySelectorAll('[id^="chart-"]').forEach(el=>{{
...
function ddAll(id){{
  ...
  const allVals=[];
  items.forEach(i=>{{i.classList.add('checked');i.querySelector('.custom-checkbox').textContent='✓';allVals.push(i.dataset.val);}});
  const btn=document.getElementById('btn-'+id);
  const label=btn?btn.querySelector('.select-value'):null;
  if(label) label.textContent='All selected';
  const widget=document.querySelector('[data-widget-id="'+id+'"]');
  if(widget&&widget._onChange) widget._onChange(allVals);
}}
function ddNone(id){{
  const items=document.querySelectorAll('#list-'+id+' .dropdown-item');
  const firstVal=items[0]?items[0].dataset.val:null;
  const keptVals=firstVal?[firstVal]:[];
  items.forEach((i,idx)=>{{
  ...
  const widget=document.querySelector('[data-widget-id="'+id+'"]');
  if(widget&&widget._onChange) widget._onChange(keptVals);
}}
...
document.querySelectorAll('.custom-dropdown, .custom-select-btn').forEach(el=>el.addEventListener('click',e=>e.stopPropagation()));
...
function renderDashboard(){{
  ...
  document.getElementById('kpi-grid').innerHTML=kpis.map((k,i)=>`
    <div class="kpi-card" style="border-top:3px solid ${{k.col}}">
  ...
  if(pv.length===0){{
    document.getElementById('chart-nps-prov').innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:220px;color:var(--text-3);font-size:12px">Tidak ada data provinsi</div>';
  }} else {{
...
function renderBranch(){{
  ...
  if(br.length===0){{
    document.getElementById('chart-branch-dot').innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:200px;color:var(--text-3);font-size:12px">Pilih filter untuk menampilkan data cabang</div>';
    document.getElementById('rank-top5').innerHTML='';
    document.getElementById('rank-bot5').innerHTML='';
    return;
  }}
  ...
  const dotH=Math.max(460,br.length*24);
  document.getElementById('chart-branch-dot').style.height=dotH+'px';
...
function renderTouchpoint(){{
  ...
  if(qw.length>0){{
    ...
    document.getElementById('chart-quickwin').innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:280px;color:var(--text-3);font-size:12px">Tidak ada atribut Quick Win</div>';
  }}
}}
...
def generate_ai_context(data: Dict[str, Any]) -> str:
    ...
    nps_comp_list = data.get('nps_comp', [])
    nps_xyz  = nps_comp_list[0]['nps_score'] if len(nps_comp_list) > 0 else 'N/A'
    nps_komp = nps_comp_list[1]['nps_score'] if len(nps_comp_list) > 1 else 'N/A'

    # Kumpulkan Quick Wins Prioritas Teratas
    ipa_cats = data.get('ipa_cats', {})
    qw_list = [item for sublist in ipa_cats.values() for item in sublist if item.get('kuadran') == 'Quick Win']
    ...
    driver_list = data.get('driver_analysis', [])
    if driver_list:
        top_drivers_list = sorted(driver_list, key=lambda x: abs(x.get('correlation', 0)), reverse=True)[:3]
        driver_str = "; ".join([f"{d['touchpoint']} (r={d['correlation']})" for d in top_drivers_list if 'touchpoint' in d and 'correlation' in d])
    else:
        driver_str = "Data driver belum tersedia"

    # Rangkuman Waktu Tunggu Antrean jika ada
    wait_str = "Data antrean stabil"
    wait_list = data.get('wait_time', [])
    if wait_list:
        top_wait = sorted(wait_list, key=lambda x: x.get('count', 0), reverse=True)[:2]
        wait_str = ", ".join([f"{w.get('panel','?')} {w.get('wait_category','?')} ({w.get('pct','?')}%)" for w in top_wait])
...
- Komparasi Market Share: Skor NPS Bank XYZ {nps_xyz} vs Kompetitor Pasar {nps_komp}.
"""
...
def render_ai_analyst_interface(data_context: str) -> None:
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "Halo! Saya sudah membaca seluruh data analitik survei Bank XYZ. Bagian mana yang ingin Anda bedah secara mendalam?"}
        ]
...
def main() -> None:
    ...
    components.html(html_dashboard, height=780, scrolling=True)
